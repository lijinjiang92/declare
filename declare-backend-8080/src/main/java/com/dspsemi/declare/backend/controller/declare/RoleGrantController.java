...
public class RoleGrantController extends BaseController {
	 ...
	 * 列表页面
	 */
	public DataPage<RoleGrant> list(DataPage<RoleGrant> dataPage,Model model) throws Exception {
		return roleGrantService.page(dataPage.getPageNo(), dataPage.getPageSize());
	}   
	 ...
	 * 执行新增
	 */
	public void adding(RoleGrant roleGrant, HttpServletResponse response) throws Exception {
		roleGrantService.add(roleGrant);
	...
	public void update(String id, Model model) throws Exception {		
		model.addAttribute("roleGrant", load(id));
	}
	/**
	 * 详情
	 */
	public RoleGrant load(String id) throws Exception {
		Assert.notNull(id, "主键参数不能为空");
		return roleGrantService.load(id);
	}
	/**
	 * 执行修改
	 */
	public void updating(RoleGrant roleGrant, HttpServletResponse response) throws Exception {
		int count = roleGrantService.update(roleGrant);
		if(count > 0)
			callback(closeDialog(), response);
		else
	...
	public CodeMsg remove(String[] ids) throws Exception {
		return gt0(roleGrantService.remove(ids));
	}
}
