 ...
 */
@Controller
public class DdYearController extends BaseController {

	@Resource
	private DdYearService ddYearService;
	
	/**
	 * 列表页面
	 */
	public DataPage<DdYear> list(DataPage<DdYear> dataPage,Model model) throws Exception {
		return ddYearService.page(dataPage.getPageNo(), dataPage.getPageSize());
	}   
	 ...
	 * 执行新增
	 */
	public void adding(DdYear ddYear, HttpServletResponse response) throws Exception {
		ddYearService.add(ddYear);
	...
	public void update(String id, Model model) throws Exception {		
		model.addAttribute("ddYear", load(id));
	}
	/**
	 * 详情
	 */
	public DdYear load(String id) throws Exception {
		Assert.notNull(id, "主键参数不能为空");
		return ddYearService.load(id);
	}
	/**
	 * 执行修改
	 */
	public void updating(DdYear ddYear, HttpServletResponse response) throws Exception {
		int count = ddYearService.update(ddYear);
		if(count > 0)
			callback(closeDialog(), response);
		else
	...
	public CodeMsg remove(String[] ids) throws Exception {
		return gt0(ddYearService.remove(ids));
	}
}
